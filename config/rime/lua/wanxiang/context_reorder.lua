...
-- 1..10-Gram：最近 1 至 10 次真实上屏内容 -> 当前真实上屏词
-- 保留原 1/2-Gram 数据键，长上下文优先，未命中时回退短上下文。
-- 数字后量词轻量调频：P 只捕获数字状态，F 负责候选置前
-- 回头码掉头：编码中退格后重新输入原码时交换前两个合法候选
-- 上屏后立即退格：回滚刚写入的 1..10-Gram 记录
...
local context_state = {
    ...
    learn_context_key = nil,
    learn_front = {},
    learn_ready = false,
    -- 同一上下文读周期内缓存候选的精确 1..10-Gram fetch 结果；不引入 prefix query。
    score_context_key = nil,
    score_cache = {},
}
...
-- history 按上屏先后排列，始终最多 10 项。中文词不含分号，键无歧义。
local function copy_history(history)
    local copy = {}
    for i = 1, #history do copy[i] = history[i] end
    return copy
end

local function context_codes(number_context)
    if number_context then return {ONE_PREFIX .. NUMBER_CONTEXT} end
    local codes = {}
    local history = context_state.history
    for n = 1, #history do
        codes[n] = tostring(n) .. KEY_SEP .. table.concat(history, KEY_SEP, #history - n + 1)
    end
    return codes
end

local function context_key(codes)
    return codes[#codes]
end

local function clear_context_score_cache()
    clear_table(context_state.score_cache)
    context_state.score_context_key = nil
end
...
-- 每个候选最多精确读取 10 条；同一上下文重复输入时复用计数。
local function get_context_counts(db, text, codes)
    local key = context_key(codes)
    if context_state.score_context_key ~= key then
        clear_context_score_cache()
        context_state.score_context_key = key
    end
    local cached = context_state.score_cache[text]
    if cached then return cached.counts, cached.tier end
    local counts, tier = {}, 0
    for n, code in ipairs(codes) do
        local count = fetch_record(db, code, text)
        counts[n] = math_max(0, count)
        if counts[n] > 0 then tier = n end
    end
    context_state.score_cache[text] = {counts = counts, tier = tier}
    return counts, tier
end

-- 10 个分支的学习限制保持原样；与上下文深度 10 相互独立。
local function begin_learning_snapshot(codes)
    clear_learning_snapshot()
    context_state.learn_context_key = context_key(codes)
    context_state.learn_ready = #codes > 0
...
local function rollback_last_commit(env)
    ...
    context_state.history = copy_history(env.undo_history or {})
...
function P.init(env)
    ...
    env.commit_cb = function(ctx)
        ...
        context_state.after_number = false
        if #context_state.history > 0 and CONFIG.CONTEXT_TIMEOUT_MS > 0 and (current_time - context_state.last_commit_time) > CONFIG.CONTEXT_TIMEOUT_MS then reset_context() end
        ...
        env.undo_history = copy_history(context_state.history)
        ...
        if db then
            local codes
            if was_after_number then
                codes = CLASSIFIER_LOOKUP[text] and context_codes(true) or {}
            else
                codes = context_codes(false)
            end
            if #codes > 0 then
                local snapshot_matches = context_state.learn_ready
                    and context_state.learn_context_key == context_key(codes)
                local counts, tier = get_context_counts(db, text, codes)
                local can_add = snapshot_matches
                    and context_state.learn_allow_new
                    and context_state.learn_seen[text] == true
                local at_front = snapshot_matches and context_state.learn_front[text] == true
                -- 稳定首选不继续加权，但为缺失的长上下文建立第一条记录。
                for n, code in ipairs(codes) do
                    if (tier == 0 and can_add)
                        or (tier > 0 and (not at_front or counts[n] == 0)) then
                        update_memory_record(db, env.undo_before, env.undo_after, code, text)
                    end
                end
            end
        end
        ...
        clear_context_score_cache()
        table.insert(context_state.history, text)
        if #context_state.history > MAX_CONTEXT then table.remove(context_state.history, 1) end
    ...
    env.delete_cb = function(ctx)
        ...
        local ok, err = pcall(function()
            ...
            if context_state.after_number then
                ...
                for _, code in ipairs(context_codes(false)) do
                    mark_record_deleted(db, code, word)
                end
            end
        end)
...
local function collect_scored_prefix(next_candidate, db, codes, classifier_mode, limit, target_end)
    local entries = {}
    local boundary_cand = nil
    local first_classifier = nil
    local first_tier = nil
    local first_counts = nil
    ...
    while #entries < limit do
        ...
        if index == 1 then
            first_classifier, first_tier, first_counts = classifier, tier, counts
        elseif not needs_sort then
            needs_sort = classifier ~= first_classifier or tier ~= first_tier
            for n = 1, #codes do
                if counts[n] ~= first_counts[n] then needs_sort = true; break end
            end
        end
        ...
        remember_snapshot_candidate(text, tier > 0)
    end
...
local function sort_scored_prefix(entries, classifier_mode)
    sort(entries, function(a, b)
        ...
        for n = #a.counts, 1, -1 do
            if a.counts[n] ~= b.counts[n] then return a.counts[n] > b.counts[n] end
        end
...
function F.func(input, env)
    ...
    local codes = context_codes(do_classifier)
    local do_context = #codes > 0
    ...
    local entries, boundary_cand, needs_sort = collect_scored_prefix(
        next_candidate, db, codes, do_classifier, scan_limit, target_end
    )

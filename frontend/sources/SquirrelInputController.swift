...
final class SquirrelInputController: IMKInputController {
  ...
  private let ghost = GhostCompletion()
  private let candidateGrid = CandidateGrid()
  private var gridInput = ""
  ...
  override func handle(_ event: NSEvent!, client sender: Any!) -> Bool {
    ...
    if event.type == .keyDown, event.keyCode == 48,
       ...
       rimeAPI.get_input(session).map({ $0.pointee != 0 }) ?? false {
      // While composing, keep Tab inside the field; do not send it to Rime.
      return true
    }
    if ghost.handle(event, client: self.client) { return true }
    if handleDirectSlash(event) { return true }
    if handleGrid(event) { return true }
    let reserveCompositionTab = event.type == .keyDown && event.keyCode == 48
      && modifiers.intersection([.command, .control, .option, .shift]).isEmpty
      && (rimeAPI.get_input(session).map { $0.pointee != 0 } ?? false)
    ...
    if event.type == .keyDown, event.keyCode != 53, !candidateGrid.isVisible,
       rimeAPI.get_input(session).map({ $0.pointee != 0 }) ?? false {
      ghost.cancel()
    }
    // Browsers treat an unhandled Tab as “focus next and select its contents”.
    // Keep Tab inside the current field whenever a live caret is present.
    return handled || reserveCompositionTab || reserveFieldTab(event)
  }

  private func reserveFieldTab(_ event: NSEvent) -> Bool {
    // IMK only delivers keys while attached to the current field. Do not require
    // selectedRange: WebKit/Blink often omit it, and an unhandled Tab then
    // focuses the next control and selects its contents.
    event.type == .keyDown && event.keyCode == 48
      && event.modifierFlags.intersection([.command, .control, .option, .shift]).isEmpty
      && client != nil
  }

  private func handleDirectSlash(_ event: NSEvent) -> Bool {
    // Match the produced character so / also works on non-US keyboard layouts;
    // application shortcuts such as Command+/ keep their normal path.
    guard event.type == .keyDown, event.characters == "/", client != nil,
    ...
    if !input.isEmpty {
      if rimeAPI.commit_composition(session) {
        rimeConsumeCommittedText()
      } else {
        // Preserve unfinished input even if this schema cannot commit a candidate.
        rimeAPI.clear_composition(session)
        commit(string: input)
      }
    }
    if rimeAPI.get_input(session).map({ $0.pointee != 0 }) == true {
      rimeAPI.clear_composition(session)
    }
    commit(string: "/")
    rimeUpdate()
  ...
  private func handleGrid(_ event: NSEvent) -> Bool {
    guard event.type == .keyDown else { return false }
    let modifiers = event.modifierFlags.intersection([.command, .control, .option, .shift])
    let digit = event.charactersIgnoringModifiers.flatMap(Int.init)
    if candidateGrid.isVisible {
      let current = rimeAPI.get_input(session).map { String(cString: $0) } ?? ""
      if current != gridInput { candidateGrid.close(); return false }
      if modifiers.isEmpty && event.keyCode == 53 {
        candidateGrid.close(); rimeUpdate(); return true
      }
      if modifiers.isEmpty && event.keyCode == 51 {
        if candidateGrid.hasColumn { candidateGrid.back() } else { candidateGrid.close(); rimeUpdate() }
        return true
      }
      if modifiers.isEmpty, let digit {
        if (1...5).contains(digit) { candidateGrid.digit(digit) } else { NSSound.beep() }
        return true
      }
      candidateGrid.close()
      rimeUpdate()
    ...
    guard modifiers.isEmpty, digit == 1, let client,
          !rimeAPI.get_option(session, "ascii_mode") else { return false }
    let input = rimeAPI.get_input(session).map { String(cString: $0) } ?? ""
    guard !input.isEmpty else { return false }
    var iterator = RimeCandidateListIterator()
    guard rimeAPI.candidate_list_begin(session, &iterator) else { return false }
    var entries: [String] = []
    while entries.count < 25 && rimeAPI.candidate_list_next(&iterator) {
      entries.append(iterator.candidate.text.map { String(cString: $0) } ?? "")
    }
    ...
    candidateGrid.open(entries: entries, at: rect) { [weak self] index in
      guard let self else { return }
      let current = self.rimeAPI.get_input(self.session).map { String(cString: $0) } ?? ""
      self.candidateGrid.close()
      if current == self.gridInput { _ = self.rimeAPI.select_candidate(self.session, index) }
      self.rimeUpdate()
    }
    return true
  }
  ...
  func acceptSystemTab() {
    guard let event = NSEvent.keyEvent(
      with: .keyDown, location: .zero, modifierFlags: [],
      timestamp: ProcessInfo.processInfo.systemUptime, windowNumber: 0, context: nil,
      characters: "\t", charactersIgnoringModifiers: "\t", isARepeat: false, keyCode: 48
    ) else { return }
    _ = handle(event, client: client)
  }

  override func activateServer(_ sender: Any!) {
    self.client ?= sender as? IMKTextInput
    GhostTabTap.shared.activate(self)
    if let client { ghost.activate(client: client) }
...
private extension SquirrelInputController {
  ...
  func rimeConsumeCommittedText() {
    ...
    if rimeAPI.get_commit(session, &commitText) {
      if let text = commitText.text {
        commit(string: String(cString: text), selected: true)
      }
  ...
  func rimeUpdate() {
    ...
    if rimeAPI.get_context(session, &ctx) {
      ...
      showPanel(preedit: inlinePreedit ? "" : preedit,
                selRange: selRange,
  ...
  func commit(string: String, selected: Bool = false) {
    candidateGrid.close()
    guard let client = client else { return }
    // print("[DEBUG] commitString: \(string)")
    ghost.willCommit(string, client: client)
    client.insertText(string, replacementRange: .empty)
    preedit = ""
    hidePalettes()
    ghost.committed(string, client: client, selected: selected)
  }
  ...
  func showPanel(preedit: String, selRange: NSRange, caretPos: Int, candidates: [String], comments: [String], labels: [String], highlighted: Int, page: Int, lastPage: Bool) {
    if candidateGrid.isVisible { NSApp.squirrelAppDelegate.panel?.hide(); return }
